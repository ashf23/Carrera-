const materias = [
  // Formato: [ID, Nombre, Semestre, [prerrequisitos]]
...
for (let semestre = 1; semestre <= 11; semestre++) {
  ...
  materias.filter(m => m[2] === semestre).forEach(m => {
    ...
    divMat.className = "materia" + (m[3].length ? " bloqueada" : "");
    divMat.textContent = m[1];
    divMat.id = `mat-${m[0]}`;
    divMat.onclick = () => aprobar(m[0]);
